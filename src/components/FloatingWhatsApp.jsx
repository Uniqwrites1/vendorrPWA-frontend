...
const FloatingWhatsApp = () => {
  ...
  useEffect(() => {
    // Load WhatsApp link from backend settings
    const loadWhatsAppLink = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/settings/whatsapp`);
        if (response.ok) {
          const data = await response.json();
          setWhatsappLink(data.whatsapp_link || 'https://wa.me/qr/EKAYKJ7XOVOTP1');
        } else {
          // Fallback to default link
          setWhatsappLink('https://wa.me/qr/EKAYKJ7XOVOTP1');
        }
      } catch (error) {
  ...
  return (
    <a
      ...
      className={`fixed bottom-6 right-6 z-50 bg-green-500 hover:bg-green-600 text-white rounded-full p-4 shadow-lg hover:shadow-xl transition-all duration-300 transform ${
